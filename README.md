...
ComfyUI-JMCAI 是一个为 ComfyUI 设计的多平台 AI 节点插件，主打**高性能、低侵入、自动化**。目前支持**火山引擎 Doubao/Seedream 4.0** API 以及高效的**局部裁切工作流**工具。
...
| 节点名称 | 功能描述 | 特性 |
|:---|:---|:---|
| **局部裁切预处理** | 自动提取遮罩区域 | 支持 **部分运行 (Partial Run)** 预览 |
| **局部裁切贴回** | 精确还原图像 | 对齐缩放后的差异 |
| **图像组合批次 (多重)** | 动态多图合并 | 自动规格化对齐 |
...
### 局部裁切：部分运行 (Partial Run) 高效预览

在 `局部裁切预处理` 节点中，开启“仅预览”选项后：
- **子图提取**：插件自动提取当前节点及其上游依赖，构建临时任务。
- **低侵入性**：不会修改或静音下游节点的状态，不污染工作流。
- **极速反馈**：仅运行必要节点，跳过下游耗时的生成和保存环节，实现近乎实时的参数微调预览。
